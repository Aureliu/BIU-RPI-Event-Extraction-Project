...
public class BundledSignals implements Serializable {
	...
	static {
		System.err.println("??? BundledSignals: Not supporting args yet as well!");
	}
...
//	public Set<ScorerData> argumentScorers;
	...
	public Map<Integer, List<Map<Integer, Map<ScorerData, SignalInstance>>>> triggerSignals;
...
//		this.argumentScorers = perceptron.argumentScorers;
		...
		this.triggerSignals = new FinalKeysMap<Integer, List<Map<Integer, Map<ScorerData, SignalInstance>>>>();
	...
	public void absorb(BundledSignals other) {
		for (Iterator<Entry<Integer, List<Map<Integer, Map<ScorerData, SignalInstance>>>>> iter1 = other.triggerSignals.entrySet().iterator(); iter1.hasNext();) {
			Entry<Integer, List<Map<Integer, Map<ScorerData, SignalInstance>>>> entry1 = iter1.next();
			if (!this.triggerSignals.containsKey(entry1.getKey())) {
				this.triggerSignals.put(entry1.getKey(), entry1.getValue());
			}
			else {
				List<Map<Integer, Map<ScorerData, SignalInstance>>> this1 = this.triggerSignals.get(entry1.getKey());
				int n=0;
				for (Iterator<Map<Integer, Map<ScorerData, SignalInstance>>> iter2 = entry1.getValue().iterator(); iter2.hasNext();) {
					Map<Integer, Map<ScorerData, SignalInstance>> elem2 = iter2.next();
					Map<Integer, Map<ScorerData, SignalInstance>> this2 = this1.get(n);
					n++;
					
					for (Iterator<Entry<Integer, Map<ScorerData, SignalInstance>>> iter3 = elem2.entrySet().iterator(); iter3.hasNext();) {
						Entry<Integer, Map<ScorerData, SignalInstance>> entry3 = iter3.next();
